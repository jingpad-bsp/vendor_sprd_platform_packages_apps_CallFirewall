...
public class BlackFragment extends BaseFragment{
    ...
    // SPRD: Not allow to selectAll in batch operation for bug 652826.
    ...
    /* SPRD: add for bug 750224 @{ */
    private static final String BUNDLE_RESULT = "result";
    private static final String BLOCKCALL_TYPE = "blackcall_type";
    /* @} */
    ...
    void setMenuItemState() {
        ...
        if (mDeleteState) {
            ...
            // SPRD: modify for Bug 761791
            mDeleteMenuItem.setEnabled(mCurrentCursorCount != 0);
        }
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case R.id.black_menu_delete_confirm:
                /* SPRD: Not allow to selectAll in batch operation for bug 652826. @{ */
                if (!canCurrentUserOpenCallFireWall(mContext)) {
                    showToast(getString(R.string.refuse_after_close_blocked_storage));
                } else if (mBatchOperation) {
                    showToast(getString(R.string.adding_blacklist_hold_on));
                } else {
    ...
    void showDialogByType(int type){
        ...
        if (type == DELETE_ALL_DIALOG) {
            ...
            /* SPRD: add for bug657693 @{ */
            mDialog = mDetailBuilder.create();
            mDialog.show();
            /* @} */
        }
        mDialogType = type;
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent
        ...
        if (requestCode == REQUEST_CODE_BLACK_EDIT) {
            ...
            updataBlackList();
        /* SPRD: add for bug 750224 & 757290 & 760789 & 762238 @{ */
        } else if (requestCode == REQUEST_CODE_PICK) {
            Bundle bundle = mData.getExtras();
            HashMap<String, String> contacts;
            String number;
            if ((bundle.getString(BLOCKCALL_TYPE) != null)
                    && (null != mData.getSerializableExtra(BUNDLE_RESULT))) {
                boolean isValidPhoneNum = false;
                contacts = (HashMap<String, String>) mData.getSerializableExtra(BUNDLE_RESULT);
                ...
                if (isValidPhoneNum) {
                    Toast.makeText(mContext, R.string.multi_block_incorrect_num_hint,
                            Toast.LENGTH_SHORT).show();
                }
                execute(mContext, contacts.size());
            }
        } else {
            HashMap<String, String> contacts;
    ...
    /* SPRD: add for bug 760789 @{ */
    private void execute(Context context, int size) {
        mProgressUtil = new ProgressUtil(context, size, ProgressType.BLACKLIST_ADD, this);
    ...
    public void doInBack(ProgressType type) {
        ...
        if (type == ProgressType.BLACKLIST_ADD) {
            ...
            while (i.hasNext()) {
                ...
                /* SPRD: add for bug 750224 @{ */
                Log.d(TAG, "doInBackground: origNumber = " + origNumber + ", " +
                        Mms.isPhoneNumber(origNumber));
                if (Mms.isPhoneNumber(origNumber)) {
                    // SPRD: Bug 601543 Can add duplicate black number
                    ...
                    // SPRD: Bug 601543 Can add duplicate black number
                    values.put(BlockedNumberContract
                            .BlockedNumbers.COLUMN_ORIGINAL_NUMBER, origNumber);
                    values.put(BLOCK_TYPE,
                            Integer.parseInt(mData.getExtras().getString(BLOCKCALL_TYPE)));
                    values.put(MIN_MATCH, PhoneNumberUtils.toCallerIDMinMatch(normalizedNumber));
                    values.put(NAME, val);
                    operations.add(ContentProviderOperation
                            .newInsert(BlockedNumberContract.BlockedNumbers.CONTENT_URI)
                            .withValues(values).withYieldAllowed(false).build());
                    icount++;
                    if (icount >= 20) {
            ...
            mData = null;
            // SPRD: add for bug 757290.
            Log.d(TAG, "doInBackground: BLACKLIST_ADD - set mData null");
            /* SPRD: add for bug 624314. @{ */
            if (mSharedPreferences != null) {
                mSharedPreferences.edit()
                    .putBoolean("batchOperation", false)
    ...
    private boolean isBatchOperation() {
        if (mSharedPreferences != null) {
            return mSharedPreferences.getBoolean("batchOperation", false);
        } else {
            return false;
        }
    }

    private void clearDoneMenu() {
        if (mCurrentCursorCount == 0) {
            mMonitorSelectAll = false;
            mDeleteState = false;
            mMarkForDelete.clear();
            if (mDeleteConfirmMenuItem != null && mCancelMenuItem != null) {
                mDeleteConfirmMenuItem.setVisible(false);
                mCancelMenuItem.setVisible(false);
            }
            setMenuItemState();
